    ...
    <AppShell>
      ...
      <div className="grid min-h-[640px] gap-6 xl:grid-cols-[1fr_320px]">
        <section className="card flex flex-col overflow-hidden">
          ...
          <div className="flex-1 space-y-5 overflow-auto bg-surface p-5">
          ...
          <div className="border-t border-slate-200 bg-white p-4">
            <div className="mb-3 flex flex-wrap gap-2">
              {["Explain simply", "Give an example", "Test me", "Summarize"].map((prompt) => (
                <button key={prompt} className="focus-ring rounded-full border border-slate-200 px-3 py-1.5 text-sm text-on-surface-variant">{prompt}</button>
              ))}
            </div>
        ...
        <aside className="card h-fit p-5">
          ...
          <div className="mt-4 space-y-3">
            {["Sampling Methodology Manual.pdf", "Field Survey Procedures.docx"].map((source) => (
              <div key={source} className="rounded-lg border border-slate-200 bg-surface p-3">
